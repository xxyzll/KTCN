...
def voc_eval(detpath, annopath, imagesetfile, classname, ovthresh=0.5,
              use_07_metric=False, known_classes=None):
...
class OWODEvaluator(DatasetEvaluator):
    """
    ...
    def __init__(self, dataset_name, cfg=None):
        ...
        self._anno_file_template = os.path.join(meta.dataset_root, "Annotations", "{}.xml")
        self._image_set_path = os.path.join(meta.dataset_root, "ImageSets", "Main", meta.file_name + ".txt")
        self._class_names = meta.thing_classes
        self._is_2007 = False
        self._cpu_device = torch.device("cpu")
        self._logger = logging.getLogger("detectron2."+__name__)
        if cfg is not None:
    ...
    def process(self, inputs, outputs):
        for input, output in zip(inputs, outputs):
            ...
            boxes = instances.pred_boxes.tensor.numpy()             # (num_prediction, 4)  list
            scores = instances.scores.tolist()                      # (num_prediction)     list 
            classes = instances.pred_classes.tolist()               # (num_presiction)     list
